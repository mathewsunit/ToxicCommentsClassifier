...
//    nbPredicted.show()
  ...
  def writeToOutPut(sc: SparkSession, trainData: DataFrame, testData: DataFrame, testSet: DataFrame,
                          destPath: String): Unit = {
    println("Building Naive Bayes model")
    val toxicNBModel = getNBModel(trainData.withColumnRenamed("toxic", "label"))
    val toxicPredictions = getPredictionForModel(toxicNBModel, testData, "toxic_predicted")
...
//    mlpPredicted.show()
    ...
    val fs = FileSystem.get(sc.sparkContext.hadoopConfiguration)
    val path: Path = new Path(destPath+"/MLBAccuracy.txt")
    if (fs.exists(path)) {
      fs.delete(path, true)
    }
    val dataOutputStream: FSDataOutputStream = fs.create(path)
    ...
    printAccuracy(mlpPredicted, "toxic", bw)
    printAccuracy(mlpPredicted, "severe_toxic", bw)
    printAccuracy(mlpPredicted, "obscene", bw)
    printAccuracy(mlpPredicted, "threat", bw)
    printAccuracy(mlpPredicted, "insult", bw)
    printAccuracy(mlpPredicted, "identity_hate", bw)
    //please try the below code. We need to print the confusion matrix for this using MulticlassMetrics
    bw.write(SparkUtils.evaluateModel(mlpPredicted.select("toxic", "toxic_predicted").rdd.map{ case Row(prediction: Double, label: Double) => (label, prediction) },"Toxic"))
    bw.write(SparkUtils.evaluateModel(mlpPredicted.select("severe_toxic", "severe_toxic_predicted").rdd.map{ case Row(prediction: Double, label: Double) => (label, prediction) },"Severe Toxic"))
    bw.write(SparkUtils.evaluateModel(mlpPredicted.select("obscene", "obscene_predicted").rdd.map{ case Row(prediction: Double, label: Double) => (label, prediction) },"Obscene"))
    bw.write(SparkUtils.evaluateModel(mlpPredicted.select("threat", "threat_predicted").rdd.map{ case Row(prediction: Double, label: Double) => (label, prediction) },"Threat"))
    bw.write(SparkUtils.evaluateModel(mlpPredicted.select("insult", "insult_predicted").rdd.map{ case Row(prediction: Double, label: Double) => (label, prediction) },"Insult"))
    bw.write(SparkUtils.evaluateModel(mlpPredicted.select("identity_hate", "identity_hate_predicted").rdd.map{ case Row(prediction: Double, label: Double) => (label, prediction) },"Identity Hate"))
    bw.close()
  }
